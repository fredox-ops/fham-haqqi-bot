...
const ArticleCard = ({ code, article, title, excerpt }: Props) => (
  <div className="paper rounded-2xl p-7 relative overflow-hidden">
    <div className="flex items-start justify-between mb-4 relative z-10">
      <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] text-gold/90">
        <BookOpen className="h-3.5 w-3.5" />
        <span>{code}</span>
      </div>
      <span className="font-justice text-xs text-muted-foreground">Art. {article}</span>
    </div>
    <h4 className="font-display text-xl mb-2 relative z-10">{title}</h4>
